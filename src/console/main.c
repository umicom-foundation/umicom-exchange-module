...
#include "umicom/exchange/workspace.h"
...
int main(void)
{
    ...
    const UmiExperienceLayoutDefinition *layout =
        umi_exchange_workspace_default();
    const UmiExperienceFeatureDefinition *next =
        umi_exchange_workspace_next_feature();

    if (definition == NULL ||
        umi_exchange_application_status(&status) != UMI_STATUS_OK ||
        layout == NULL) {
        fputs("Umicom Commodity Exchange: Framework experience unavailable\n", stderr);
        return 1;
    }
